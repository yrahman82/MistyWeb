...
function ResetPasswordInner() {
  ...
  useEffect(() => {
    if (!token) { setStage("error"); setErrorMsg(t("noToken")); return; }
    fetch(`${API}/api/auth/verify-reset-token?token=${encodeURIComponent(token)}`)
      .then(r => r.json())
      .then(d => {
        if (d.valid) setStage("form");
        else { setStage("error"); setErrorMsg(d.error ?? t("invalidLink")); }
      })
      .catch(() => { setStage("error"); setErrorMsg(t("serverUnreachable")); });
  }, [token, t]);

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (password !== confirm) { setErrorMsg(t("passwordsDontMatch")); return; }
    if (password.length < 8) { setErrorMsg(t("passwordTooShort")); return; }
    setErrorMsg("");
    setLoading(true);
    try {
      ...
      else { setErrorMsg(d.error ?? t("somethingWrong")); }
    } catch {
  ...
  return (
    <main className="min-h-screen bg-[#0A1628] flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white/5 border border-white/10 rounded-2xl p-8">
        ...
        {stage === "verifying" && (
          <div className="text-center text-white/60 text-sm">{t("verifying")}</div>
        )}

        {stage === "error" && (
          <div className="text-center">
            <div className="text-red-400 text-sm mb-4">{errorMsg}</div>
            <Link href="/" className="text-[#4FC3F7] text-sm hover:underline">
              {t("requestNewLink")}
            </Link>
          </div>
        )}
        ...
        {stage === "form" && (
          <form onSubmit={submit} className="space-y-4">
            <h2 className="text-white font-semibold text-lg mb-1">{t("formHeading")}</h2>
            <p className="text-white/50 text-sm mb-4">{t("formSubtitle")}</p>
